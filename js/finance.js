...
const Finance = {
    ...
    renderChart(finances) {
        ...
        // Draw bars
        const barWidth = chartWidth / (finances.length * 3);
        const groupWidth = chartWidth / finances.length;

        finances.forEach((f, i) => {
            const x = padding + groupWidth * i;
            ...
            // Profit bar (blue)
            const profitHeight = Math.abs(profitData[i]) * scale;
            ctx.fillStyle = profitData[i] >= 0 ? '#3B82F6' : '#F59E0B';
            ctx.fillRect(x + barWidth * 2, padding + chartHeight - profitHeight, barWidth, profitHeight);
        });
        ...
        ctx.fillRect(width - 200, legendY + 50, 15, 15);
        ctx.fillStyle = '#374151';
        ctx.fillText('Lợi nhuận', width - 180, legendY + 62);
    ...
    exportReport() {
        Utils.showToast('Đang xuất báo cáo tài chính...', 'info');
        setTimeout(() => {
    ...
    exportMonthReport(id) {
        Utils.showToast('Đang xuất báo cáo tháng...', 'info');
        setTimeout(() => {
            Utils.showToast('Đã xuất file PDF', 'success');
        }, 1000);
    },
